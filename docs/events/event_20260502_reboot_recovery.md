...
- The script waits for `docker-compose.yml` and `grafana/dashboards/thesmart-dlp-file-evidence.json`, runs `docker compose up -d --remove-orphans`, then verifies the Grafana container sees `/var/lib/grafana/dashboards/thesmart-dlp-file-evidence.json`.
...
- `docker exec warroom-grafana test -f /var/lib/grafana/dashboards/thesmart-dlp-file-evidence.json`: pass.
