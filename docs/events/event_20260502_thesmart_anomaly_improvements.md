...
將 `docs/ops/thesmart-anomaly-readiness.md` 的改進建議正式寫成 plan 並執行，集中強化利善美智能 dashboard 與資料收集能力，讓後續可做超大流量、異常登入、登入錯誤、異常時間下載等偵測。
...
- Prior audit: `docs/ops/thesmart-anomaly-readiness.md`.
...
See `plans/20260502_thesmart_anomaly_improvements/tasks.md`.
...
- `python3 -m json.tool thesmart/config/sources.json`: pass.
...
- Collector restarted; Loki confirmed `source_channel="auth_log"` events (`auth_failure`, `session_opened`, `session_closed`) and `source_channel="network_socket"` `network_socket_snapshot` events for `nas_host="thesmart"`.
...
- Retry validation after registry rebuild:
  - `sum(count_over_time({nas_host="thesmart", source_channel="auth_log"} | json | source_key="auth_log_remote" [5m]))`: 800.
  - `sum(count_over_time({nas_host="thesmart", source_channel="network_socket"} | json | source_key="network_socket_remote" [5m]))`: 8.
  - `sum(count_over_time({job="warroom-dlp-event-collector", nas_host="thesmart", action="capability_gap", source_channel="collector_capability_gap"}[2m])) or vector(0)`: 0.
