...
- Dashboard provisioning uses file providers from `grafana/provisioning/dashboards/dashboards.yml` and enables `allowUiUpdates: true` so operators can save edited provisioned dashboards from the Grafana UI. Those UI saves persist to Grafana's database only; they do not automatically write back to `grafana/dashboards/*.json`, and later provisioning-source updates can overwrite the database copy.
...
- `thesmart-recent-file-access-table` presents deduplicated recently accessed file rows from Loki payload fields only, grouped by `file_name`, `actor`, `source_app`, and `source_ip`, with evidence row count shown separately. Missing users or IPs remain empty rather than being inferred.
...
- `thesmart-nas-host-health` visualizes SSH-pulled host-health snapshots. Current-value gauges/stat panels for CPU, memory, disk, and uptime must aggregate unwrapped Loki payload fields into a single instant value because each snapshot has unique event metadata such as `event_id`; trend panels must likewise aggregate labels before plotting one logical series per metric. Service availability is a time-axis state view, not a current-value bar gauge: each service (`ssh`, `smb`, `nginx`, `httpd`, `dsm`) should appear as one row on a state timeline, with 0/1 mapped to down/up status colors.
- `warroom-dlp-insights` provides management-facing DLP evidence statistics from existing Loki payload fields only. Time-related activity is represented as a timeline/timeseries panel. Non-time aggregate views must use visualizations that match their semantics: horizontal bar charts for rankings such as actor/IP/folder/file/source/protocol/country counts, and tables only for rows requiring per-item inspection. Ranking/proportion queries must use instant aggregation over the selected range rather than range-table output, so Grafana does not expose misleading Time table rows. Empty charts/tables mean the field/source is not covered in the selected range or no matching events occurred; they must not imply placeholder data. File access ranking panels must be scoped to explicit file-access actions and must not rank arbitrary payload strings just because a `file_name` field is present. Protocol/country ranking panels must rename Grafana query value fields into user-facing labels such as `通訊協定` / `國家` and `事件數`; raw field names such as `Value #A` or `Volume #A` must not appear in the dashboard. Internal collector capability/coverage-gap panels should not appear on this insights dashboard; keep those engineering-health signals in operational/debug surfaces when needed.
...
- Additional monitored target: `thesmart` (display name `TheSmartAI`) points to `nas.wuyang.co` with SSH user `wuyangadmin` for read-only File Station transfer DB, Synology Drive activity DB, and NAS home-scope log metadata collection. Warroom does not deploy containers, persistent agents, exporters, Promtail, or Grafana on this NAS.
...
- `tools/drive_activity_adapter.py`: read-only Synology Drive activity adapter that streams over SSH stdin, reads `log-db.sqlite` plus `view/<id>/view-db.sqlite`, and emits conservative `drive_file_activity` / validated web action evidence with file size metadata when available.
...
The `drive_remote` collector source uses the same read-only SSH payload posture to poll Drive `log_table` rows and correlate `view_id + p1` to `node_table` metadata. Unknown or Drive Client sync-like action codes are emitted conservatively as `drive_file_activity`; they are used for large-transfer observability when `size_bytes` is available, not presented as explicit downloads unless controlled validation supports that action mapping.
