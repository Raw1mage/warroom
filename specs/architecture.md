...
- Additional monitored target: `lishanmei` (`利善美智能`) points to `nas.wuyang.co` with SSH user `wuyangadmin` for read-only File Station transfer DB and NAS home-scope log metadata collection. Warroom does not deploy containers, persistent agents, exporters, Promtail, or Grafana on this NAS.
- Lishanmei anomaly sources include structured auth log metadata (`auth_log_remote`) and network socket snapshots (`network_socket_remote`) for login failure/session and connection-spike readiness.
