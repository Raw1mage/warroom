...
- `warroom-dlp-insights` provides evidence statistics from existing Loki payload fields only. Time-related activity is represented as a timeline/timeseries panel; non-time views remain rankings/tables for actor/IP/protocol/file/folder, GeoIP country/region when local MMDB enrichment exists, source channels, and capability gaps. Empty ranking tables must mean the field/source is not covered in the selected range or no matching events occurred; they must not imply placeholder data.
